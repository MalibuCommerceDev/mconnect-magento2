    ...
    public function execute()
    {
        ...
        foreach ($collection->getItems() as $order) {
            ...
            try {
                ...
                $this->queue->create()->add(\MalibuCommerce\MConnect\Model\Queue\Order::CODE, \MalibuCommerce\MConnect\Model\Queue::ACTION_EXPORT, $websiteId, 0, $order->getId(), [], $scheduledAt);
            } catch (\Throwable $e) {
        ...
        if ($countNonAddedOrder && $countMassQueue) {
            $this->messageManager->addErrorMessage(__('%1 order(s) cannot be canceled.', $countNonAddedOrder));
        ...
        if ($countMassQueue) {
            $this->messageManager->addSuccessMessage(__('We canceled %1 order(s).', $countMassQueue));
        }
