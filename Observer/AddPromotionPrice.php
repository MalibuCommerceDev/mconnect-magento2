    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($collection->getSize() > 0 ) {
            ...
            foreach ($collection as $product) {
                $prepareProducts[$product->getSku()] = self::PRODUCT_QTY_FOR_IMPORT;
                ...
                if ($product->getTypeId() == \Magento\ConfigurableProduct\Model\Product\Type\Configurable::TYPE_CODE) {
                    ...
                    $select->where('link_table.parent_id IN (?)', [$product->getId()]);

                    $simpleProducts = $connection->fetchAssoc($select);
                    if (count($simpleProducts) > 0) {
                        foreach ($simpleProducts as $simpleProduct) {
                            $prepareProducts[$simpleProduct['sku']] = self::PRODUCT_QTY_FOR_IMPORT;
                        }
                    }
                }
            }
            ...
            foreach ($prepareProducts as $itemKey => $itemValue) {
